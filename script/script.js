...
var pointA = [0,0];
var pointB = [4,3];
var jarak_aktif_dragging = false;
var hold = false;
...
function GameAnimation() { 
    ...
    for (let i = 0; i < kumpulan_kayu.length; i++) {
        console.log(kumpulan_kayu[i].koordinat);
        ...
        if (jarak_aktif_dragging) {
            kumpulan_kayu[index_balok_bersentuhan].koordinat[0] = position[0] - kumpulan_kayu[index_balok_bersentuhan].panjang/2;
            kumpulan_kayu[index_balok_bersentuhan].koordinat[1] = position[1] - kumpulan_kayu[index_balok_bersentuhan].lebar/2;
        } 

        if (bersentuhan) {
            console.log(bersentuhan);
            document.onclick = function() {
                pos_awal_mouse = position;
                selection_counter = selection_counter + 1;
                if (selection_counter%2 == 0) {
                    enable_dragging = true;
                    index_balok_bersentuhan = i;
                    //pos_awal_mouse = position;
                    //console.log(position);
                }
                else {
                    enable_dragging = false;
                    jarak_aktif_dragging = false;
                    hold = false;
                }
            }
            if(enable_dragging) {
